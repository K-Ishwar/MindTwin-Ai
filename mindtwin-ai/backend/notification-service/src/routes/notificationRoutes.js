...
// ── Internal (API key protected — no JWT) ─────────────────────────────────────
router.post('/send',                ctrl.sendNotification);   // single send
router.post('/send-bulk',           ctrl.sendBulk);           // multicast send
router.post('/send-otp-email',      ctrl.sendOTPEmail);       // OTP dispatch from auth-service
router.post('/send-weekly-digest',  ctrl.sendWeeklyDigest);   // cron trigger
